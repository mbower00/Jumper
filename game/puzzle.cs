using System;

    ///Holds a list of words to randomly pull words from, holds the pulled word, 
    ///and determines if the guess is correct or not
    public class Puzzle{

        Random random = new Random();
        List<char> wordLetters = new List<char>();

        public Puzzle()
        {

        }


        /// <summary>
        /// Takes word from WordList and turns it into a list of characters
        /// </summary>
        /// <param name="newWord"></param>
        /// <returns>list of characters made from the word</returns>
        public List<char> LettersNeeded(string newWord)
        {
            

            for (int i = 0; i < newWord.Length; i++)
            {
                wordLetters.Add(newWord[i]);
            }
            
            return wordLetters;

        }

        /// <summary>
        /// Compares the player's guess to the word used to see if the letter 
        /// they guess is in the word
        /// </summary>
        /// <param name="wordLetters"></param>
        /// <returns>the index number(s) of the letter guess if correct</returns>
        public List<int> GetGuesses(char guess)
        {
            int letNum = (int) guess % 32;
            List<int> correctGuess = new List<int>();

            for (int i = 0; i < wordLetters.Count; i++ )
             {
                if (letNum == (int) wordLetters[i] % 32)
                {
                    
                    correctGuess.Add(i);
                    

                }
                 
                else
                {
                    correctGuess.Add(-1);
                }
             }
             return correctGuess;

        }

        /// <summary>
        /// Holds a list of words to be pulled from
        /// </summary>
        /// <returns>one word to be played with</returns>
        public string WordList()
        {

            List<string> wordBank = new List<string>();  
            wordBank.Add("imminent");  
            wordBank.Add("giant"); 
            wordBank.Add("glitter"); 
            wordBank.Add("clothes"); 
            wordBank.Add("soccer"); 
            wordBank.Add("thunder"); 
            wordBank.Add("lightning"); 
            wordBank.Add("train"); 
            wordBank.Add("bike");
            //Words (with some changes) from: https://gist.github.com/deekayen/4148741
            //The largest word's length is 11
            wordBank.Add("the");
            wordBank.Add("of");
            wordBank.Add("to");
            wordBank.Add("and");
            wordBank.Add("in");
            wordBank.Add("is");
            wordBank.Add("it");
            wordBank.Add("you");
            wordBank.Add("that");
            wordBank.Add("he");
            wordBank.Add("was");
            wordBank.Add("for");
            wordBank.Add("on");
            wordBank.Add("are");
            wordBank.Add("with");
            wordBank.Add("as");
            wordBank.Add("his");
            wordBank.Add("they");
            wordBank.Add("be");
            wordBank.Add("at");
            wordBank.Add("one");
            wordBank.Add("have");
            wordBank.Add("this");
            wordBank.Add("from");
            wordBank.Add("or");
            wordBank.Add("had");
            wordBank.Add("by");
            wordBank.Add("not");
            wordBank.Add("word");
            wordBank.Add("but");
            wordBank.Add("what");
            wordBank.Add("some");
            wordBank.Add("we");
            wordBank.Add("can");
            wordBank.Add("out");
            wordBank.Add("other");
            wordBank.Add("were");
            wordBank.Add("all");
            wordBank.Add("there");
            wordBank.Add("when");
            wordBank.Add("up");
            wordBank.Add("use");
            wordBank.Add("your");
            wordBank.Add("how");
            wordBank.Add("said");
            wordBank.Add("an");
            wordBank.Add("each");
            wordBank.Add("she");
            wordBank.Add("which");
            wordBank.Add("do");
            wordBank.Add("their");
            wordBank.Add("time");
            wordBank.Add("if");
            wordBank.Add("will");
            wordBank.Add("way");
            wordBank.Add("about");
            wordBank.Add("many");
            wordBank.Add("then");
            wordBank.Add("them");
            wordBank.Add("write");
            wordBank.Add("would");
            wordBank.Add("like");
            wordBank.Add("so");
            wordBank.Add("these");
            wordBank.Add("her");
            wordBank.Add("long");
            wordBank.Add("make");
            wordBank.Add("thing");
            wordBank.Add("see");
            wordBank.Add("him");
            wordBank.Add("two");
            wordBank.Add("has");
            wordBank.Add("look");
            wordBank.Add("more");
            wordBank.Add("day");
            wordBank.Add("could");
            wordBank.Add("go");
            wordBank.Add("come");
            wordBank.Add("did");
            wordBank.Add("number");
            wordBank.Add("sound");
            wordBank.Add("no");
            wordBank.Add("most");
            wordBank.Add("people");
            wordBank.Add("my");
            wordBank.Add("over");
            wordBank.Add("know");
            wordBank.Add("water");
            wordBank.Add("than");
            wordBank.Add("call");
            wordBank.Add("first");
            wordBank.Add("who");
            wordBank.Add("may");
            wordBank.Add("down");
            wordBank.Add("side");
            wordBank.Add("been");
            wordBank.Add("now");
            wordBank.Add("find");
            wordBank.Add("any");
            wordBank.Add("new");
            wordBank.Add("work");
            wordBank.Add("part");
            wordBank.Add("take");
            wordBank.Add("get");
            wordBank.Add("place");
            wordBank.Add("made");
            wordBank.Add("live");
            wordBank.Add("where");
            wordBank.Add("after");
            wordBank.Add("back");
            wordBank.Add("little");
            wordBank.Add("only");
            wordBank.Add("round");
            wordBank.Add("man");
            wordBank.Add("year");
            wordBank.Add("came");
            wordBank.Add("show");
            wordBank.Add("every");
            wordBank.Add("good");
            wordBank.Add("me");
            wordBank.Add("give");
            wordBank.Add("our");
            wordBank.Add("under");
            wordBank.Add("name");
            wordBank.Add("very");
            wordBank.Add("through");
            wordBank.Add("just");
            wordBank.Add("form");
            wordBank.Add("sentence");
            wordBank.Add("great");
            wordBank.Add("think");
            wordBank.Add("say");
            wordBank.Add("help");
            wordBank.Add("low");
            wordBank.Add("line");
            wordBank.Add("differ");
            wordBank.Add("turn");
            wordBank.Add("cause");
            wordBank.Add("much");
            wordBank.Add("mean");
            wordBank.Add("before");
            wordBank.Add("move");
            wordBank.Add("right");
            wordBank.Add("boy");
            wordBank.Add("old");
            wordBank.Add("too");
            wordBank.Add("same");
            wordBank.Add("tell");
            wordBank.Add("does");
            wordBank.Add("set");
            wordBank.Add("three");
            wordBank.Add("want");
            wordBank.Add("air");
            wordBank.Add("well");
            wordBank.Add("also");
            wordBank.Add("play");
            wordBank.Add("small");
            wordBank.Add("end");
            wordBank.Add("put");
            wordBank.Add("home");
            wordBank.Add("read");
            wordBank.Add("hand");
            wordBank.Add("port");
            wordBank.Add("large");
            wordBank.Add("spell");
            wordBank.Add("add");
            wordBank.Add("even");
            wordBank.Add("land");
            wordBank.Add("here");
            wordBank.Add("must");
            wordBank.Add("big");
            wordBank.Add("high");
            wordBank.Add("such");
            wordBank.Add("follow");
            wordBank.Add("act");
            wordBank.Add("why");
            wordBank.Add("ask");
            wordBank.Add("men");
            wordBank.Add("change");
            wordBank.Add("went");
            wordBank.Add("light");
            wordBank.Add("kind");
            wordBank.Add("off");
            wordBank.Add("need");
            wordBank.Add("house");
            wordBank.Add("picture");
            wordBank.Add("try");
            wordBank.Add("us");
            wordBank.Add("again");
            wordBank.Add("animal");
            wordBank.Add("point");
            wordBank.Add("mother");
            wordBank.Add("world");
            wordBank.Add("near");
            wordBank.Add("build");
            wordBank.Add("self");
            wordBank.Add("earth");
            wordBank.Add("father");
            wordBank.Add("head");
            wordBank.Add("stand");
            wordBank.Add("own");
            wordBank.Add("page");
            wordBank.Add("should");
            wordBank.Add("country");
            wordBank.Add("found");
            wordBank.Add("answer");
            wordBank.Add("school");
            wordBank.Add("grow");
            wordBank.Add("study");
            wordBank.Add("still");
            wordBank.Add("learn");
            wordBank.Add("plant");
            wordBank.Add("cover");
            wordBank.Add("food");
            wordBank.Add("sun");
            wordBank.Add("four");
            wordBank.Add("between");
            wordBank.Add("state");
            wordBank.Add("keep");
            wordBank.Add("eye");
            wordBank.Add("never");
            wordBank.Add("last");
            wordBank.Add("let");
            wordBank.Add("thought");
            wordBank.Add("city");
            wordBank.Add("tree");
            wordBank.Add("cross");
            wordBank.Add("farm");
            wordBank.Add("hard");
            wordBank.Add("start");
            wordBank.Add("might");
            wordBank.Add("story");
            wordBank.Add("saw");
            wordBank.Add("far");
            wordBank.Add("sea");
            wordBank.Add("draw");
            wordBank.Add("left");
            wordBank.Add("late");
            wordBank.Add("run");
            wordBank.Add("dime");
            wordBank.Add("while");
            wordBank.Add("press");
            wordBank.Add("close");
            wordBank.Add("night");
            wordBank.Add("real");
            wordBank.Add("life");
            wordBank.Add("few");
            wordBank.Add("north");
            wordBank.Add("open");
            wordBank.Add("seem");
            wordBank.Add("together");
            wordBank.Add("next");
            wordBank.Add("white");
            wordBank.Add("children");
            wordBank.Add("begin");
            wordBank.Add("got");
            wordBank.Add("walk");
            wordBank.Add("example");
            wordBank.Add("ease");
            wordBank.Add("paper");
            wordBank.Add("group");
            wordBank.Add("always");
            wordBank.Add("music");
            wordBank.Add("those");
            wordBank.Add("both");
            wordBank.Add("mark");
            wordBank.Add("often");
            wordBank.Add("letter");
            wordBank.Add("until");
            wordBank.Add("mile");
            wordBank.Add("river");
            wordBank.Add("car");
            wordBank.Add("feet");
            wordBank.Add("care");
            wordBank.Add("second");
            wordBank.Add("book");
            wordBank.Add("carry");
            wordBank.Add("took");
            wordBank.Add("science");
            wordBank.Add("eat");
            wordBank.Add("room");
            wordBank.Add("friend");
            wordBank.Add("began");
            wordBank.Add("idea");
            wordBank.Add("fish");
            wordBank.Add("mountain");
            wordBank.Add("stop");
            wordBank.Add("once");
            wordBank.Add("base");
            wordBank.Add("hear");
            wordBank.Add("horse");
            wordBank.Add("cut");
            wordBank.Add("sure");
            wordBank.Add("watch");
            wordBank.Add("color");
            wordBank.Add("face");
            wordBank.Add("wood");
            wordBank.Add("main");
            wordBank.Add("enough");
            wordBank.Add("plain");
            wordBank.Add("girl");
            wordBank.Add("usual");
            wordBank.Add("young");
            wordBank.Add("ready");
            wordBank.Add("above");
            wordBank.Add("ever");
            wordBank.Add("red");
            wordBank.Add("list");
            wordBank.Add("though");
            wordBank.Add("feel");
            wordBank.Add("talk");
            wordBank.Add("bird");
            wordBank.Add("soon");
            wordBank.Add("body");
            wordBank.Add("dog");
            wordBank.Add("family");
            wordBank.Add("direct");
            wordBank.Add("pose");
            wordBank.Add("leave");
            wordBank.Add("song");
            wordBank.Add("measure");
            wordBank.Add("door");
            wordBank.Add("product");
            wordBank.Add("black");
            wordBank.Add("short");
            wordBank.Add("numeral");
            wordBank.Add("class");
            wordBank.Add("wind");
            wordBank.Add("question");
            wordBank.Add("happen");
            wordBank.Add("complete");
            wordBank.Add("ship");
            wordBank.Add("area");
            wordBank.Add("half");
            wordBank.Add("rock");
            wordBank.Add("order");
            wordBank.Add("fire");
            wordBank.Add("south");
            wordBank.Add("problem");
            wordBank.Add("piece");
            wordBank.Add("told");
            wordBank.Add("knew");
            wordBank.Add("pass");
            wordBank.Add("since");
            wordBank.Add("top");
            wordBank.Add("whole");
            wordBank.Add("king");
            wordBank.Add("space");
            wordBank.Add("heard");
            wordBank.Add("best");
            wordBank.Add("hour");
            wordBank.Add("better");
            wordBank.Add("true");
            wordBank.Add("during");
            wordBank.Add("hundred");
            wordBank.Add("five");
            wordBank.Add("remember");
            wordBank.Add("step");
            wordBank.Add("early");
            wordBank.Add("hold");
            wordBank.Add("west");
            wordBank.Add("ground");
            wordBank.Add("interest");
            wordBank.Add("reach");
            wordBank.Add("fast");
            wordBank.Add("verb");
            wordBank.Add("sing");
            wordBank.Add("listen");
            wordBank.Add("six");
            wordBank.Add("table");
            wordBank.Add("travel");
            wordBank.Add("less");
            wordBank.Add("morning");
            wordBank.Add("ten");
            wordBank.Add("simple");
            wordBank.Add("several");
            wordBank.Add("vowel");
            wordBank.Add("toward");
            wordBank.Add("war");
            wordBank.Add("lay");
            wordBank.Add("against");
            wordBank.Add("pattern");
            wordBank.Add("slow");
            wordBank.Add("center");
            wordBank.Add("love");
            wordBank.Add("person");
            wordBank.Add("money");
            wordBank.Add("serve");
            wordBank.Add("appear");
            wordBank.Add("road");
            wordBank.Add("map");
            wordBank.Add("rain");
            wordBank.Add("rule");
            wordBank.Add("govern");
            wordBank.Add("pull");
            wordBank.Add("cold");
            wordBank.Add("notice");
            wordBank.Add("voice");
            wordBank.Add("unit");
            wordBank.Add("power");
            wordBank.Add("town");
            wordBank.Add("fine");
            wordBank.Add("certain");
            wordBank.Add("fly");
            wordBank.Add("fall");
            wordBank.Add("lead");
            wordBank.Add("cry");
            wordBank.Add("dark");
            wordBank.Add("machine");
            wordBank.Add("note");
            wordBank.Add("wait");
            wordBank.Add("plan");
            wordBank.Add("figure");
            wordBank.Add("star");
            wordBank.Add("box");
            wordBank.Add("noun");
            wordBank.Add("field");
            wordBank.Add("rest");
            wordBank.Add("correct");
            wordBank.Add("able");
            wordBank.Add("pound");
            wordBank.Add("done");
            wordBank.Add("beauty");
            wordBank.Add("drive");
            wordBank.Add("stood");
            wordBank.Add("contain");
            wordBank.Add("front");
            wordBank.Add("teach");
            wordBank.Add("week");
            wordBank.Add("final");
            wordBank.Add("gave");
            wordBank.Add("green");
            wordBank.Add("oh");
            wordBank.Add("quick");
            wordBank.Add("develop");
            wordBank.Add("ocean");
            wordBank.Add("warm");
            wordBank.Add("free");
            wordBank.Add("minute");
            wordBank.Add("strong");
            wordBank.Add("special");
            wordBank.Add("mind");
            wordBank.Add("behind");
            wordBank.Add("clear");
            wordBank.Add("tail");
            wordBank.Add("produce");
            wordBank.Add("fact");
            wordBank.Add("street");
            wordBank.Add("inch");
            wordBank.Add("multiply");
            wordBank.Add("nothing");
            wordBank.Add("course");
            wordBank.Add("stay");
            wordBank.Add("wheel");
            wordBank.Add("full");
            wordBank.Add("force");
            wordBank.Add("blue");
            wordBank.Add("object");
            wordBank.Add("decide");
            wordBank.Add("surface");
            wordBank.Add("deep");
            wordBank.Add("moon");
            wordBank.Add("island");
            wordBank.Add("foot");
            wordBank.Add("system");
            wordBank.Add("busy");
            wordBank.Add("test");
            wordBank.Add("record");
            wordBank.Add("boat");
            wordBank.Add("common");
            wordBank.Add("gold");
            wordBank.Add("possible");
            wordBank.Add("plane");
            wordBank.Add("stead");
            wordBank.Add("dry");
            wordBank.Add("wonder");
            wordBank.Add("laugh");
            wordBank.Add("thousand");
            wordBank.Add("ago");
            wordBank.Add("ran");
            wordBank.Add("check");
            wordBank.Add("game");
            wordBank.Add("shape");
            wordBank.Add("equate");
            wordBank.Add("hot");
            wordBank.Add("miss");
            wordBank.Add("brought");
            wordBank.Add("heat");
            wordBank.Add("snow");
            wordBank.Add("tire");
            wordBank.Add("bring");
            wordBank.Add("yes");
            wordBank.Add("distant");
            wordBank.Add("fill");
            wordBank.Add("east");
            wordBank.Add("paint");
            wordBank.Add("language");
            wordBank.Add("among");
            wordBank.Add("grand");
            wordBank.Add("ball");
            wordBank.Add("yet");
            wordBank.Add("wave");
            wordBank.Add("drop");
            wordBank.Add("heart");
            wordBank.Add("am");
            wordBank.Add("present");
            wordBank.Add("heavy");
            wordBank.Add("dance");
            wordBank.Add("engine");
            wordBank.Add("position");
            wordBank.Add("arm");
            wordBank.Add("wide");
            wordBank.Add("sail");
            wordBank.Add("material");
            wordBank.Add("size");
            wordBank.Add("vary");
            wordBank.Add("settle");
            wordBank.Add("speak");
            wordBank.Add("weight");
            wordBank.Add("general");
            wordBank.Add("ice");
            wordBank.Add("matter");
            wordBank.Add("circle");
            wordBank.Add("pair");
            wordBank.Add("include");
            wordBank.Add("divide");
            wordBank.Add("syllable");
            wordBank.Add("felt");
            wordBank.Add("perhaps");
            wordBank.Add("pick");
            wordBank.Add("sudden");
            wordBank.Add("count");
            wordBank.Add("square");
            wordBank.Add("reason");
            wordBank.Add("length");
            wordBank.Add("represent");
            wordBank.Add("art");
            wordBank.Add("subject");
            wordBank.Add("region");
            wordBank.Add("energy");
            wordBank.Add("hunt");
            wordBank.Add("probable");
            wordBank.Add("bed");
            wordBank.Add("brother");
            wordBank.Add("egg");
            wordBank.Add("ride");
            wordBank.Add("cell");
            wordBank.Add("believe");
            wordBank.Add("fraction");
            wordBank.Add("forest");
            wordBank.Add("sit");
            wordBank.Add("race");
            wordBank.Add("window");
            wordBank.Add("store");
            wordBank.Add("summer");
            wordBank.Add("train");
            wordBank.Add("sleep");
            wordBank.Add("prove");
            wordBank.Add("lone");
            wordBank.Add("leg");
            wordBank.Add("exercise");
            wordBank.Add("wall");
            wordBank.Add("catch");
            wordBank.Add("mount");
            wordBank.Add("wish");
            wordBank.Add("sky");
            wordBank.Add("board");
            wordBank.Add("joy");
            wordBank.Add("winter");
            wordBank.Add("sat");
            wordBank.Add("written");
            wordBank.Add("wild");
            wordBank.Add("instrument");
            wordBank.Add("kept");
            wordBank.Add("glass");
            wordBank.Add("grass");
            wordBank.Add("cow");
            wordBank.Add("job");
            wordBank.Add("edge");
            wordBank.Add("sign");
            wordBank.Add("visit");
            wordBank.Add("past");
            wordBank.Add("soft");
            wordBank.Add("fun");
            wordBank.Add("bright");
            wordBank.Add("gas");
            wordBank.Add("weather");
            wordBank.Add("month");
            wordBank.Add("million");
            wordBank.Add("bear");
            wordBank.Add("finish");
            wordBank.Add("happy");
            wordBank.Add("hope");
            wordBank.Add("flower");
            wordBank.Add("clothe");
            wordBank.Add("strange");
            wordBank.Add("gone");
            wordBank.Add("jump");
            wordBank.Add("baby");
            wordBank.Add("eight");
            wordBank.Add("village");
            wordBank.Add("meet");
            wordBank.Add("root");
            wordBank.Add("buy");
            wordBank.Add("raise");
            wordBank.Add("solve");
            wordBank.Add("metal");
            wordBank.Add("whether");
            wordBank.Add("push");
            wordBank.Add("seven");
            wordBank.Add("paragraph");
            wordBank.Add("third");
            wordBank.Add("shall");
            wordBank.Add("held");
            wordBank.Add("hair");
            wordBank.Add("describe");
            wordBank.Add("cook");
            wordBank.Add("floor");
            wordBank.Add("either");
            wordBank.Add("result");
            wordBank.Add("burn");
            wordBank.Add("hill");
            wordBank.Add("safe");
            wordBank.Add("cat");
            wordBank.Add("century");
            wordBank.Add("consider");
            wordBank.Add("type");
            wordBank.Add("law");
            wordBank.Add("bit");
            wordBank.Add("coast");
            wordBank.Add("copy");
            wordBank.Add("phrase");
            wordBank.Add("silent");
            wordBank.Add("tall");
            wordBank.Add("sand");
            wordBank.Add("soil");
            wordBank.Add("roll");
            wordBank.Add("temperature");
            wordBank.Add("finger");
            wordBank.Add("industry");
            wordBank.Add("value");
            wordBank.Add("fight");
            wordBank.Add("lie");
            wordBank.Add("beat");
            wordBank.Add("excite");
            wordBank.Add("natural");
            wordBank.Add("view");
            wordBank.Add("sense");
            wordBank.Add("ear");
            wordBank.Add("else");
            wordBank.Add("quite");
            wordBank.Add("broke");
            wordBank.Add("case");
            wordBank.Add("middle");
            wordBank.Add("kill");
            wordBank.Add("son");
            wordBank.Add("lake");
            wordBank.Add("moment");
            wordBank.Add("scale");
            wordBank.Add("loud");
            wordBank.Add("spring");
            wordBank.Add("observe");
            wordBank.Add("child");
            wordBank.Add("straight");
            wordBank.Add("consonant");
            wordBank.Add("nation");
            wordBank.Add("dictionary");
            wordBank.Add("milk");
            wordBank.Add("speed");
            wordBank.Add("method");
            wordBank.Add("organ");
            wordBank.Add("pay");
            wordBank.Add("age");
            wordBank.Add("section");
            wordBank.Add("dress");
            wordBank.Add("cloud");
            wordBank.Add("surprise");
            wordBank.Add("quiet");
            wordBank.Add("stone");
            wordBank.Add("tiny");
            wordBank.Add("climb");
            wordBank.Add("cool");
            wordBank.Add("design");
            wordBank.Add("poor");
            wordBank.Add("lot");
            wordBank.Add("experiment");
            wordBank.Add("bottom");
            wordBank.Add("key");
            wordBank.Add("iron");
            wordBank.Add("single");
            wordBank.Add("stick");
            wordBank.Add("flat");
            wordBank.Add("twenty");
            wordBank.Add("skin");
            wordBank.Add("smile");
            wordBank.Add("crease");
            wordBank.Add("hole");
            wordBank.Add("trade");
            wordBank.Add("melody");
            wordBank.Add("trip");
            wordBank.Add("office");
            wordBank.Add("receive");
            wordBank.Add("row");
            wordBank.Add("mouth");
            wordBank.Add("exact");
            wordBank.Add("symbol");
            wordBank.Add("die");
            wordBank.Add("least");
            wordBank.Add("trouble");
            wordBank.Add("shout");
            wordBank.Add("except");
            wordBank.Add("wrote");
            wordBank.Add("seed");
            wordBank.Add("tone");
            wordBank.Add("join");
            wordBank.Add("suggest");
            wordBank.Add("clean");
            wordBank.Add("break");
            wordBank.Add("lady");
            wordBank.Add("yard");
            wordBank.Add("rise");
            wordBank.Add("bad");
            wordBank.Add("blow");
            wordBank.Add("oil");
            wordBank.Add("blood");
            wordBank.Add("touch");
            wordBank.Add("grew");
            wordBank.Add("cent");
            wordBank.Add("mix");
            wordBank.Add("team");
            wordBank.Add("wire");
            wordBank.Add("cost");
            wordBank.Add("lost");
            wordBank.Add("brown");
            wordBank.Add("wear");
            wordBank.Add("garden");
            wordBank.Add("equal");
            wordBank.Add("sent");
            wordBank.Add("choose");
            wordBank.Add("fell");
            wordBank.Add("fit");
            wordBank.Add("flow");
            wordBank.Add("fair");
            wordBank.Add("bank");
            wordBank.Add("collect");
            wordBank.Add("save");
            wordBank.Add("control");
            wordBank.Add("decimal");
            wordBank.Add("gentle");
            wordBank.Add("woman");
            wordBank.Add("captain");
            wordBank.Add("practice");
            wordBank.Add("separate");
            wordBank.Add("difficult");
            wordBank.Add("doctor");
            wordBank.Add("please");
            wordBank.Add("protect");
            wordBank.Add("noon");
            wordBank.Add("whose");
            wordBank.Add("locate");
            wordBank.Add("ring");
            wordBank.Add("character");
            wordBank.Add("insect");
            wordBank.Add("caught");
            wordBank.Add("period");
            wordBank.Add("indicate");
            wordBank.Add("radio");
            wordBank.Add("spoke");
            wordBank.Add("atom");
            wordBank.Add("human");
            wordBank.Add("history");
            wordBank.Add("effect");
            wordBank.Add("electric");
            wordBank.Add("expect");
            wordBank.Add("crop");
            wordBank.Add("modern");
            wordBank.Add("element");
            wordBank.Add("hit");
            wordBank.Add("student");
            wordBank.Add("corner");
            wordBank.Add("party");
            wordBank.Add("supply");
            wordBank.Add("bone");
            wordBank.Add("rail");
            wordBank.Add("imagine");
            wordBank.Add("provide");
            wordBank.Add("agree");
            wordBank.Add("thus");
            wordBank.Add("capital");
            wordBank.Add("wise");
            wordBank.Add("chair");
            wordBank.Add("danger");
            wordBank.Add("fruit");
            wordBank.Add("rich");
            wordBank.Add("thick");
            wordBank.Add("soldier");
            wordBank.Add("process");
            wordBank.Add("operate");
            wordBank.Add("guess");
            wordBank.Add("necessary");
            wordBank.Add("sharp");
            wordBank.Add("wing");
            wordBank.Add("create");
            wordBank.Add("neighbor");
            wordBank.Add("wash");
            wordBank.Add("bat");
            wordBank.Add("rather");
            wordBank.Add("crowd");
            wordBank.Add("corn");
            wordBank.Add("compare");
            wordBank.Add("poem");
            wordBank.Add("string");
            wordBank.Add("bell");
            wordBank.Add("depend");
            wordBank.Add("meat");
            wordBank.Add("rub");
            wordBank.Add("tube");
            wordBank.Add("famous");
            wordBank.Add("dollar");
            wordBank.Add("stream");
            wordBank.Add("fear");
            wordBank.Add("sight");
            wordBank.Add("thin");
            wordBank.Add("triangle");
            wordBank.Add("planet");
            wordBank.Add("hurry");
            wordBank.Add("chief");
            wordBank.Add("colony");
            wordBank.Add("clock");
            wordBank.Add("mine");
            wordBank.Add("tie");
            wordBank.Add("enter");
            wordBank.Add("major");
            wordBank.Add("fresh");
            wordBank.Add("search");
            wordBank.Add("send");
            wordBank.Add("yellow");
            wordBank.Add("gun");
            wordBank.Add("allow");
            wordBank.Add("print");
            wordBank.Add("dead");
            wordBank.Add("spot");
            wordBank.Add("desert");
            wordBank.Add("suit");
            wordBank.Add("current");
            wordBank.Add("lift");
            wordBank.Add("rose");
            wordBank.Add("continue");
            wordBank.Add("block");
            wordBank.Add("chart");
            wordBank.Add("hat");
            wordBank.Add("sell");
            wordBank.Add("success");
            wordBank.Add("company");
            wordBank.Add("subtract");
            wordBank.Add("event");
            wordBank.Add("particular");
            wordBank.Add("deal");
            wordBank.Add("swim");
            wordBank.Add("term");
            wordBank.Add("opposite");
            wordBank.Add("wife");
            wordBank.Add("shoe");
            wordBank.Add("shoulder");
            wordBank.Add("spread");
            wordBank.Add("arrange");
            wordBank.Add("camp");
            wordBank.Add("invent");
            wordBank.Add("cotton");
            wordBank.Add("born");
            wordBank.Add("determine");
            wordBank.Add("quart");
            wordBank.Add("nine");
            wordBank.Add("truck");
            wordBank.Add("noise");
            wordBank.Add("level");
            wordBank.Add("chance");
            wordBank.Add("gather");
            wordBank.Add("shop");
            wordBank.Add("stretch");
            wordBank.Add("throw");
            wordBank.Add("shine");
            wordBank.Add("property");
            wordBank.Add("column");
            wordBank.Add("molecule");
            wordBank.Add("select");
            wordBank.Add("wrong");
            wordBank.Add("gray");
            wordBank.Add("repeat");
            wordBank.Add("require");
            wordBank.Add("broad");
            wordBank.Add("prepare");
            wordBank.Add("salt");
            wordBank.Add("nose");
            wordBank.Add("plural");
            wordBank.Add("anger");
            wordBank.Add("claim");
            wordBank.Add("continent");
            wordBank.Add("oxygen");
            wordBank.Add("sugar");
            wordBank.Add("death");
            wordBank.Add("pretty");
            wordBank.Add("skill");
            wordBank.Add("women");
            wordBank.Add("season");
            wordBank.Add("solution");
            wordBank.Add("magnet");
            wordBank.Add("silver");
            wordBank.Add("thank");
            wordBank.Add("branch");
            wordBank.Add("match");
            wordBank.Add("suffix");
            wordBank.Add("especially");
            wordBank.Add("fig");
            wordBank.Add("afraid");
            wordBank.Add("huge");
            wordBank.Add("sister");
            wordBank.Add("steel");
            wordBank.Add("discuss");
            wordBank.Add("forward");
            wordBank.Add("similar");
            wordBank.Add("guide");
            wordBank.Add("experience");
            wordBank.Add("score");
            wordBank.Add("apple");
            wordBank.Add("bought");
            wordBank.Add("led");
            wordBank.Add("pitch");
            wordBank.Add("coat");
            wordBank.Add("mass");
            wordBank.Add("card");
            wordBank.Add("band");
            wordBank.Add("rope");
            wordBank.Add("slip");
            wordBank.Add("win");
            wordBank.Add("dream");
            wordBank.Add("evening");
            wordBank.Add("condition");
            wordBank.Add("feed");
            wordBank.Add("tool");
            wordBank.Add("total");
            wordBank.Add("basic");
            wordBank.Add("smell");
            wordBank.Add("valley");
            wordBank.Add("nor");
            wordBank.Add("double");
            wordBank.Add("seat");
            wordBank.Add("arrive");
            wordBank.Add("master");
            wordBank.Add("track");
            wordBank.Add("parent");
            wordBank.Add("shore");
            wordBank.Add("division");
            wordBank.Add("sheet");
            wordBank.Add("substance");
            wordBank.Add("favor");
            wordBank.Add("connect");
            wordBank.Add("post");
            wordBank.Add("spend");
            wordBank.Add("chord");
            wordBank.Add("fat");
            wordBank.Add("glad");
            wordBank.Add("original");
            wordBank.Add("share");
            wordBank.Add("station");
            wordBank.Add("dad");
            wordBank.Add("bread");
            wordBank.Add("charge");
            wordBank.Add("proper");
            wordBank.Add("bar");
            wordBank.Add("offer");
            wordBank.Add("segment");
            wordBank.Add("slave");
            wordBank.Add("duck");
            wordBank.Add("instant");
            wordBank.Add("market");
            wordBank.Add("degree");
            wordBank.Add("populate");
            wordBank.Add("chick");
            wordBank.Add("dear");
            wordBank.Add("enemy");
            wordBank.Add("reply");
            wordBank.Add("drink");
            wordBank.Add("occur");
            wordBank.Add("support");
            wordBank.Add("speech");
            wordBank.Add("nature");
            wordBank.Add("range");
            wordBank.Add("steam");
            wordBank.Add("motion");
            wordBank.Add("path");
            wordBank.Add("liquid");
            wordBank.Add("log");
            wordBank.Add("meant");
            wordBank.Add("quotient");
            wordBank.Add("teeth");
            wordBank.Add("shell");
            wordBank.Add("neck");
            wordBank.Add("faith");
            wordBank.Add("holy");         
            int newWord = random.Next(wordBank.Count);
            return wordBank[newWord];
            
        }
    }
