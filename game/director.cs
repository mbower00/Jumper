...
namespace cse210_jumper.game{
    public class Director{
      ...
      /// </summary>    
        ...
        private void GetInputs(){ /// showing the user the promts written in the terminal class and calling them here
        ...
        /// if all the "_" are removed before that they win the game
                ...
                else if (jumper.GetChuteDamage() == 5) /// call from jumper to get the damage and checking to see if the player has exceded 5 yet
                {
                    terminal.DisplayLose() /// displays the loss function called from terminal
                    break;
                } 
                if !(jumper.GetStatus().Contains("_")){ /// now we pull from jumper to see if the blanks are gone
